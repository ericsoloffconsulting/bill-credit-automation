    ...
    function loadAndParseCSV(fileId) {
        try {
            ...
            for (var i = 0; i < Math.min(lines.length, 10); i++) {
                ...
                // 1. Contains "invoiceNumber" (new CSV format from Claude)
                ...
                for (var j = 0; j < parsedLine.length; j++) {
                    ...
                    if (value) {
                        nonEmptyCount++;
                        if (value.toLowerCase() === 'invoicenumber' ||
                            value === 'invoiceNumber') {
                ...
                // Found header row if it has invoiceNumber
                if (hasInvoiceNumber) {
                    headerRowIndex = i;
                    headers = parsedLine;
            ...
            for (var i = headerRowIndex + 1; i < lines.length; i++) {
                ...
                // Check for invoiceNumber column (new CSV format from Claude)
            ...
            for (var i = 0; i < rows.length; i++) {
                var orderNo = rows[i].invoiceNumber;
    ...
    function extractOrderData(parsedData, orderNo) {
        try {
            ...
            for (var i = 0; i < parsedData.rows.length; i++) {
                if (parsedData.rows[i].invoiceNumber === orderNo) {
            ...
            var invoiceDate = firstRow.invoiceDate;
            var totalAmount = firstRow.documentTotal;
            ...
            for (var i = 0; i < orderRows.length; i++) {
                var row = orderRows[i];
                var nardaNumber = row.nardaNumber ? row.nardaNumber.trim() : null;
                ...
                // Parse totalAmount (format: "($3.39)")
                var extendedPrice = Math.abs(parseFloat(row.totalAmount.replace(/[$(),]/g, '')));

                // Get original bill number directly from column (no extraction needed)
                var originalBillNumber = row.originalBillNumber ? row.originalBillNumber.trim() : null;

                // Create line item object
                var lineItem = {
                    part: row.partNumber,
                    description: '', // No description column in new format
                    price: row.totalAmount, // Store formatted amount
                    quantity: 1, // Not tracked in new format
            ...
            return {
                ...
                invoiceDate: invoiceDate,
                totalAmount: totalAmount,
                deliveryAmount: firstRow.deliveryAmount || '$0.00',
                creditType: firstRow.creditType || '',
